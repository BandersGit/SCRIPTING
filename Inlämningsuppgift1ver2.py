...
while True:     # Loop för att slippa starta om programmet när man skriver fel input
    print("")
    input_file = Path(input("Please input the filename or filepath you would like to use: "))     # Tar input och sparar ner den som en Path variabel för vidare hantering
    
    if input_file.is_file() and input_file.suffix == ".txt":     # Använder Path funktioner för att kolla att filen finns, är en fil och har rätt filändelse

        with open(input_file, "r", encoding = "utf-8") as f:     # Öppnar filen och använder aliaset f för att hantera den
